...
/**
 * Shader constants ported from the original Compose module for use with XML views.
 * These AGSL shaders work identically in both Compose and traditional Android Views.
 */
object LiquidGlassShaders {

    /**
     * Shared SDF (Signed Distance Field) functions for rounded rectangles.
     * This is the foundation for all liquid glass effects.
     */
...
half4 main(float2 coord) {
    ...
    /**
     * Default highlight shader for directional lighting effects.
     */
    const val DEFAULT_HIGHLIGHT_SHADER = """
uniform shader content;
...
half4 main(float2 coord) {
     ...
     */
    const val AMBIENT_HIGHLIGHT_SHADER = """
uniform shader content;
...
half4 main(float2 coord) {
    ...
    /**
     * Gamma adjustment shader for color correction.
     */
    const val GAMMA_ADJUSTMENT_SHADER = """
uniform shader content;
